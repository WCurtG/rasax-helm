...
password_question() {
  ...
  [[ $INITIAL_USER_PASSWORD != "" ]] && INITIAL_USER_PASSWORD=$INITIAL_USER_PASSWORD
...
update_values() {
  cd "${HOME}/rasax-helm/scripts/Install" &&
...
deploy_helm() {
  cd "${HOME}" &&
    ...
    chmod go-r /var/snap/microk8s/2487/credentials/client.config &&
